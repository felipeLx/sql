...
# Moreover, to any of these DML statements, one of two types of triggers can be assigned 
...
# After stating we want to CREATE a TRIGGER and then indicating its name,
# we must indicate its type and the name of the table 
# we devised a “before” trigger, which will be activated whenever new data is inserted 
...
# example of a conditional. The IF statement
# The operation is terminated by the END IF phrase and a semi-colon. 
# the use of the keyword NEW. In general, it refers to a row that 
# has just been inserted or updated. In our case, after we insert a new record, 
# “NEW dot salary” will refer to the value that will 
...
# Second, in the IF conditional statement, instead of setting the new value to 0, 
# we are basically telling MySQL to keep the old value. 
# Technically, this is achieved by setting the NEW value in the “Salary”
# column to be equal to the OLD one. 
/*
	IF NEW.salary < 0 THEN 
		SET NEW.salary = OLD.salary; 
	END IF;
...
# UPDATE statement, modify the salary value of employee 10001 with another positive value.
UPDATE salaries 
SET 
    salary = 98765
WHERE
...
# Now, let’s run another UPDATE statement
UPDATE salaries 
SET 
    salary = - 50000
WHERE
...
# All right. For the moment, you know you have created only two triggers. 
# ‘info’ section of the “employees” database, you can find a tab related to triggers.
# MySQL will show you the name, the related event, table, timing, and other characteristics
# Are pre-defined system variables and system functions.
# System functions can also be called built-in functions. 
# They provide data related to the moment of the execution of a certain query.
# SYSDATE() delivers the date and time of the moment at which you have invoked this function.
SELECT SYSDATE();

# Another frequently employed function, “Date Format”, assigns a specific format to a given date.
# could extract the current date, quoting the year, the month, and the day. 
SELECT DATE_FORMAT(SYSDATE(), '%y-%m-%d') as today;

# Of course, there are many other ways in which you could format a date; 
DELIMITER $$
...
# Conceptually, this was an ‘after’ trigger that automatically added $20,000 to the salary
# it set the start date of her new contract to be the day on which you executed the insert statement.
